...
interface ActivityTimeEntriesSectionProps {
  teamId: string
  activityId: string
}

export function ActivityTimeEntriesSection({
  teamId,
  activityId,
}: ActivityTimeEntriesSectionProps) {
  ...
  const loadTimeEntries = useCallback(async () => {
    ...
    try {
      ...
      const data = await api<TaskTimeEntriesResponse>(
        `/teams/${teamId}/activities/${activityId}/time-entries?${params.toString()}`,
      )
  ...
  }, [teamId, activityId, page, selectedDate])
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Horas registradas nesta atividade e quem as fez.
          </p>
        </div>

        <div className="flex flex-col gap-2 sm:w-52">
          <Label htmlFor="activity-entries-date">Filtrar por dia</Label>
          <Input
            id="activity-entries-date"
            type="date"
            value={selectedDate}
      ...
      {isLoading ? (
        ...
        <div className="flex min-h-32 flex-col items-center justify-center gap-2 rounded-xl border border-dashed bg-muted/30 p-8 text-center">
          ...
          <p className="max-w-sm text-sm text-muted-foreground">
            Os apontamentos desta atividade para o dia selecionado aparecerão
            aqui.
          </p>
        </div>
      ) : (

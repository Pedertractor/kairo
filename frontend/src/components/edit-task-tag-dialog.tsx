...
interface EditTaskTagDialogProps {
  projectId: string
  teamId: string
...
export function EditTaskTagDialog({
  projectId,
  teamId,
  task,
  open,
  ...
  useEffect(() => {
    ...
    setSelectedTagId(task.tag?.id ?? NO_TAG)
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const currentTagId = task.tag?.id ?? null
    ...
    try {
      const payload: UpdateTaskInput = { tagId: nextTagId }
      ...
      <DialogContent>
        <form onSubmit={handleSubmit}>
          <DialogHeader>
            ...
            <DialogDescription>
              {task
                ? `Selecione a etiqueta para "${task.title}".`
                : 'Selecione a etiqueta da tarefa.'}
            </DialogDescription>
          </DialogHeader>
          ...
          <FieldGroup className="py-4">
            <Field>
              ...
              <Select
                ...
                disabled={isSubmitting || isLoadingTags || !task}
              >
